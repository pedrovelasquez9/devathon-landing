import type { Faqs } from "../interfaces/Faqs.type";

export const faqs: Faqs[] =[
    {
        title: '¿Cuánto dura una edición del Devathon?',
        answer: 'Las ediciones suelen durar un máximo de 6 semanas, pudiendo variar entre ediciones.'
    },
    {
        title: '¿Qué nivel debo tener para participar?',
        answer: '¡No hay nivel mínimo! Si bien es conveniente que tengas nociones de programación y ya estés familiarizado con algún lenguaje o stack de desarrollo, la idea del Devathon es brindar la experiencia práctica a todos los niveles, especialmente a aquellos que están empezando y estudiando.'
    },
    {
        title: '¿Por dónde me entero de las inscripciones y actividades?',
        answer: 'Todo lo relacionado al Devathon se publica y se transmite por el canal de Twitch y el servidor de Discord de la comunidad.'
    },
    {
        title: 'Si conozco otras personas interesadas, ¿podríamos armar un equipo para entrar?',
        answer: 'Si ya conoces a varias personas interesadas en participar como equipo, pueden hacerlo sin problema. Solo tienen que notificarlo en el directo donde se arman los equipos.'
    },
    {
        title: '¿Cómo se arman los equipos?',
        answer: 'Se toman en cuenta cosas como el país, horario y stack de los inscritos y se tratan de armar equipos según estas características para que les sea cómodo comunicarse y trabajar.'
    },
    {
        title: '¿Qué tipo de proyectos se desarrollan?',
        answer: 'Los proyectos son propuestos en el formulario de inscripción y, luego de armar los equipos, se asignan proyectos a cada uno de forma aleatoria.'
    },
    {
        title:'¿Qué pasa si me tengo que retirar de mi equipo en mitad del evento?',
        answer:'Todos tenemos otras actividades y asuntos personales, si debes retirarte del evento no pasa nada, solo se pide informar al equipo para que puedan adaptar su estrategia de desarrollo con respecto al número de participantes y magnitud del proyecto.'
    },
    {
        title:'¿Puedo participar en nuevas ediciones si ya he participado antes?',
        answer:'¡Por supuesto que sí!'
    },
    {
        title:'¿Cuándo comienza la próxima edición?',
        answer:'Las fechas de cada edición se publicarán en las redes de la comunidad una vez culmine la edición actual.'
    },
    {
        title:'¿Puedo unirme a mitad de una edición?',
        answer:'Si hay participantes que se han retirado, solemos abrir inscripciones en directos de Twitch para cubrir estos puestos vacíos.'
    },
]